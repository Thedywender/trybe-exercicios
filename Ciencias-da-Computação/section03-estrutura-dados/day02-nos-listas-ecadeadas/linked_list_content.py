...
class LinkedList:
    ...
    def insert_at(
        self, value, position
    ):  # complexidade O(n) inserção de um elemento em qualquer lugar da lista
        if position < 1:
            return self.insert_first(value)
    ...
    def remove_last(self):  # complexidade O(1) remoção de u item no final da lista
        if len(self) <= 1:
    ...
    def remove_at(
        self, position
    ):  # complexidade O(n) remoção de um item em qualquer lugar da lista
        if position < 1:
            return self.remove_first()
        if position >= len(self):
    ...
    def get_element_at(
        self, position
    ):  # complexidade O(n) busca um iten em qualquer local da lista
        value_returned = None
    ...
    # or another way to declare method clear
    # def clear(self):
    ...
    def __get_node_at(
        self, position
    ):  # complexidade O(n) realiza busca em qualquer lugar da lista
    ...
    def index_of(self, value):  # complexidade O(n) percorre toda a lista
        position = 1
